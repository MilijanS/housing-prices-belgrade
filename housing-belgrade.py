...
def get_jsons_from_page(web_page, page_index):
    ...
    for i, div in enumerate(mydivs):
        try:
            ...
            jsonucitan = json.loads(stringic)
...
def pickle_json_array(filename):
        
    pickle.dump(baza_jsona, open(filename, 'wb'))

def unpickle_json_array(filename):
    return pickle.load(open(filename, 'rb'))
    
#get_multiple_jsons(halo_oglasi_stanovi_beograd, 101, 200)
get_multiple_jsons(halo_oglasi_stanovi_beograd, 101, 200)
pickle_json_array('baza jsona 2.pickle')
